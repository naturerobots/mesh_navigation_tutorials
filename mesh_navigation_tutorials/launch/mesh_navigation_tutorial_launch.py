...
def generate_launch_description():
    # path to this pkg
    pkg_mesh_navigation_tutorials_sim = get_package_share_directory('mesh_navigation_tutorials_sim')
    pkg_mesh_navigation_tutorials = get_package_share_directory('mesh_navigation_tutorials')

    # Comment Alex: One can have different maps for same worlds. is this to much choice for a tutorial?
    # Launch arguments
    available_map_names = [f[:-3] for f in os.listdir(os.path.join(pkg_mesh_navigation_tutorials, 'maps')) if f.endswith('.h5')]

    launch_args = [
      DeclareLaunchArgument("map_name", 
                            description='Name of the map to be used for navigation (see mesh_navigation_tutorials\' "maps" directory).',
                            default_value=LaunchConfiguration('world_name'),
                            choices=available_map_names),
      DeclareLaunchArgument("localization_type", 
    ...
    simulation = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([pkg_mesh_navigation_tutorials_sim, 'launch', 'simulation_launch.py'])
        )
    )
    ...
    map_loc_rmcl_micp = Node(
        ...
        parameters=[{
                "use_sim_time" : True,
                "map_file": PathJoinSubstitution([pkg_mesh_navigation_tutorials, 'maps', PythonExpression(['"', map_name, '" + ".dae"'])])
                },
    ...
    move_base_flex = IncludeLaunchDescription(
        ...
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([pkg_mesh_navigation_tutorials, 'launch', 'mbf_mesh_navigation_server_launch.py'])
        ),
        launch_arguments={
          'mesh_map_path': PathJoinSubstitution([pkg_mesh_navigation_tutorials, 'maps', PythonExpression(['"', map_name, '" + ".h5"'])]),
        }.items(),
    )
    ...
    rviz = Node(
        ...
        arguments=[
            '-d', PathJoinSubstitution([pkg_mesh_navigation_tutorials, 'rviz', 'default.rviz'])
        ],
